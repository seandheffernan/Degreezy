    ...
    user_model.findOne({usertoken: token}, {})
        ...
        if (err) {
            ...
            if (!data) {
                // Make Semesters
                // Assuming semesters
                let newUser = new user_model({usertoken: token, first_name: "RPI", last_name: "Student", semesterCount: 8, semesterAdmitted: null, expectedGraduation: null, programs: [null, null]});
...
export async function buildCSV(token, callback) {
    ...
    for (let i = 0; i < user.schedule.length; i++) {
        ...
        csv += semester.courses[semester.courses.length - 1] + ' newline ';
    }
    // csv += '</div>';
    callback(csv);
}
